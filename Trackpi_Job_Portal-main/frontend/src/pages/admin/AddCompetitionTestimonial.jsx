...
const AddCompetitionTestimonial = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.jobTitle || !formData.about || !files.coverImage || !files.video) {
        return toast.error("Please fill all required fields and upload media");
    }
    ...
    const loadingToast = toast.loading("Adding competition testimonial...");
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:8000"}/api/competition-testimonials/admin`,
        {
          method: "POST",
      ...
      if (!res.ok) throw new Error(data.message || "Failed to add testimonial");
      
      toast.success("Competition testimonial added successfully", { id: loadingToast });
      navigate("/admin/competition/testimonials");
    } catch (error) {
      toast.error(error.message || "Failed to add testimonial", { id: loadingToast });
    }
  };
  ...
  return (
    <div className="p-8 bg-white min-h-screen max-w-5xl mx-auto rounded-2xl shadow-sm border border-gray-100">
      <h1 className="text-2xl font-bold mb-10 text-gray-800">Add More Testimonials</h1>
...
export default AddCompetitionTestimonial;
