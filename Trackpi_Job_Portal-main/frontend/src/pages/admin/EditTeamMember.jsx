...
const EditTeamMember = () => {
  ...
  useEffect(() => {
    const fetchMember = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/team/${id}`, {
        ...
        if (data.success) {
          ...
          setPreviews({ image: member.image?.url });
        } else {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm w-full max-w-[1100px] mx-auto flex flex-col p-8 gap-5">
      <div className="flex items-center gap-4 mb-4">
        ...
        <h1 className="text-2xl font-bold text-gray-900 leading-none">Edit Member</h1>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <div className="space-y-6">
            <div>
              ...
              <input name="name" value={formData.name} onChange={handleChange} className="border-b border-gray-200 py-2 w-full outline-none text-gray-800" />
            </div>
            <div>
              <label className="block text-[18px] mb-1 font-semibold text-black">Designation</label>
              <input name="designation" value={formData.designation} onChange={handleChange} className="border-b border-gray-200 py-2 w-full outline-none text-gray-800" />
            </div>
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="block text-[18px] mb-1 font-semibold text-black">Display Order</label>
                <input type="number" name="order" value={formData.order} onChange={handleChange} className="border-b border-gray-200 py-2 w-full outline-none text-gray-800" />
              </div>
              <div className="flex items-end pb-2 gap-2">
                <input type="checkbox" id="isActive" name="isActive" checked={formData.isActive} onChange={handleChange} className="w-4 h-4 rounded text-yellow-500 focus:ring-yellow-500" />
          ...
          <div className="space-y-6">
             <div>
               ...
               <MediaBox label="Change Image" name="image" preview={previews.image} onChange={handleFileChange} />
               ...
               <div className="flex justify-start pt-10 gap-4">
                <button type="button" onClick={() => navigate("/admin/team")} className="bg-white text-gray-700 px-12 py-2 border border-gray-300 rounded-[6px] font-medium" style={{ width: "167.6px", height: "35.3px" }}>Cancel</button>
                <button type="submit" disabled={loading} className="bg-gradient-to-b from-[#FFF0CA] to-[#FFB300] text-black px-12 py-2 rounded-[6px] font-medium shadow-sm" style={{ width: "167.6px", height: "35.3px" }}>{loading ? "Saving..." : "Update"}</button>
              </div>
             </div>
          </div>
        </div>
